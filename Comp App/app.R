             ...
             sidebarLayout(
               ...
               sidebarPanel(
                 ...
                 numericInput(inputId = "S_SLG",
                             ...
                             value = 0.5,
                             step = 0.001),
                 h5("Predictions are made using a Random Forest model with the
                    ranger package in R.")
                 ),
...
server <- function(input, output) {
  ...
  percentile %>% 
    ...
    scale_color_gradient2(high = "red", low = "steelblue", mid = "gray",
                         midpoint = 50, limits = c(0, 100)) +
    theme_classic() +
    labs(title = "Projections Percentile Rankings",
         subtitle = "For individual player seasons from 2018 - 2023") +
