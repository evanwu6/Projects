...
wOBAs_Percentile <- read_csv("Outing Percentiles.csv")
...
data <- data %>% 
  group_by(Pitcher) %>% 
  mutate(PitchCount = cumsum(Level == "D1" | HomeTeam == "ELO_PHO" | AwayTeam == "ELO_PHO")) %>% 
  ungroup()
...
# Creating folders
if (!dir.exists(paste0("Reports/Game Reports ", date))) {
  dir.create(paste0("Reports/Game Reports ", date))
}

if (!dir.exists(paste0("Reports/Game Reports ", date, "/Graphics"))) {
  dir.create(paste0("Reports/Game Reports ", date, "/Graphics"))
}
...
file_graph_pv <- paste0("Reports/Game Reports ", date, "/Graphics/", name_file, ".", "pv.png")
ggsave(filename = file_graph_pv, plot = graph_pv, 
       width = 8, height = 2.25, dpi = 300) 
...
file_table_om <- paste0("Reports/Game Reports ", date, "/Graphics/", name_file, ".", "om.png")
...
file_table_obom <- paste0("Reports/Game Reports ", date, "/Graphics/", name_file, ".", "obom.png")
...
graph_qc <- pitcher_data %>%
  ...
  mutate(SwingResult = ifelse(PitchCall == "StrikeSwinging",
         ...
         SwingResult = ifelse(PitchCall == "InPlay" & ExitSpeed >= 95,
...
graph_xwOBA <- data_advanced %>% 
  ...
  geom_text(aes(label = label), nudge_x = 0.075,
            color = "#B59A57", size = 6, 
            family = "Times New Roman", fontface = "bold") +
  geom_vline(xintercept = xwOBA,
             linetype = 2, size = 1.25, color = "#73000A") +
